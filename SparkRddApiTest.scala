...
class RddDemo {
   ...
   /**
     * 与zipWithIndex不同，因为它只是为每个数据元素提供了唯一的id，但id可能与数据元素的索引号不匹配。即使RDD分布在多个分区中，这个操作也不会启动spark作业。
     */
   @Test
   def zipWithUniqueId(): Unit = {
      val z = sc.parallelize(100 to 120, 5)
      val r = z.zipWithUniqueId()
      //res24: Array[(Int, Long)] = Array((100,0), (101,5), (102,10), (103,15), (104,1), (105,6),
      // (106,11), (107,16), (108,2), (109,7), (110,12), (111,17), (112,3), (113,8), (114,13),
      // (115,18), (116,4), (117,9), (118,14), (119,19), (120,24))
      r.collect
   }

   /**
     * 使用其元素索引zip RDD的元素。索引从0开始。如果RDD分布在多个分区中，则会启动spark作业来执行此操作。
     */
   @Test
   def zipWithIndex(): Unit = {
      val a = sc.parallelize(List("A", "B", "C"), 2)
      // res23: Array[(String, Long)] = Array((A,0), (B,1), (C,2))
      a.zipWithIndex().collect
   }

   /**
     * 与zip操作差不多，但是具有更大的灵活性和可操控性。
     */
   @Test
   def zipParititions(): Unit = {
      ...
      def myfunc(aiter: Iterator[Int], biter: Iterator[Int], citer: Iterator[Int]): Iterator[String] = {
         ...
         while (aiter.hasNext && biter.hasNext && citer.hasNext) {
            val str = aiter.next() + " " + biter.next() + " " + citer.next()
            newList ::= str
         }
         newList.iterator
      }
      // res20: Array[String] = Array(2 12 102, 1 11 101, 0 10 100, 5 15 105, 4 14 104, 3 13 103, 9 19 109, 8 18 108, 7 17 107, 6 16 106)
      a.zipPartitions(b, c)(myfunc).sortBy(x => x).collect
   }
   ...
   @Test
   def zip(): Unit = {
      val a = sc.parallelize(List("dog", "cat", "pig"), 2)
      val b = sc.parallelize(1 to 3, 2)
      //res19: Array[(String, Int)] = Array((dog,1), (cat,2), (pig,3))
      //分区数量必须是一致的。
      a.zip(b).collect
   ...
   def values(): Unit = {
      ...
      val c = b.keyBy(_.length)
      //res12: Array[String] = Array(dog, cat, ape, salmon, gnu)
      c.values.collect
      // 另一种写法
      //res12: Array[String] = Array(dog, cat, ape, salmon, gnu)
      b.map(x => (x.length(), x)).values.collect
   }
   ...
   /**
     * 和Reduce几乎一样，只有一点不同的是，treeReduce是作用在n个RDD上面。
     */
   @Test
   def treeReduce(): Unit = {
   ...
   /**
     * 聚合函数
     *
     * @param zeroValue 初始值，作用于每个分区，参与seqOp函数的计算过程。但“不”参与combOp函数的计算过程
     * @param seqOp     分区之内的操作函数
     * @param combOp    聚合不同分区（被seqOp已经处理过的分区）的操作函数
     */
   @Test
   def treeAggregate(): Unit = {
      ...
      //分区之内调用math.max(_,_)函数计算，分区之间采用_+_函数进行计算
      // 相当远math.max([1,2,3], 0) + math.max([4,5,6], 0) = 3 + 6 = 9
      z.treeAggregate(4)(math.max(_, _), _ + _)
      // 相当远math.max([1,2,3], 10) + math.max([4,5,6], 10) = 10 + 10 = 20
      z.treeAggregate(10)(math.max(_, _), _ + _)
   }
